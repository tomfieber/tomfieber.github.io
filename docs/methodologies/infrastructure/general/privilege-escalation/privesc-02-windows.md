...
|`Set-DnsServerGlobalQueryBlockList -Enable $false -ComputerName dc01.example.local`|Disabling the global query block list|
|`Add-DnsServerResourceRecordA -Name wpad -ZoneName example.local -ComputerName dc01.example.local -IPv4Address 10.10.14.3`|Adding a WPAD record|
