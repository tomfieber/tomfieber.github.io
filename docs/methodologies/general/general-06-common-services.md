...
|**Command**|**Description**|
|---|---|
|`dig AXFR @ns1.example.htb example.htb`|Perform an AXFR zone transfer attempt against a specific name server.|
|`subfinder -d example.com -v`|Brute-forcing subdomains.|
|`host support.example.com`|DNS lookup for the specified subdomain.|
...
|`host -t A mail1.example.htb.`|DNS lookup of the IPv4 address for the specified subdomain.|
|`telnet 10.10.110.20 25`|Connect to the SMTP server.|
|`smtp-user-enum -M RCPT -U userlist.txt -D example.htb -t 10.129.203.7`|SMTP user enumeration using the RCPT command against the specified host.|
...
|`swaks --from notifications@example.com --to employees@example.com --header 'Subject: Notification' --body 'Message' --server 10.10.11.213`|Testing the SMTP service for the open-relay vulnerability.|