...
```bash
httpx -l domain-subs-final -status-code -title -content-length -web-server -asn -location -no-color -follow-redirects -t 15 -ports 80,8080,443,8443,4443,8888 -no-fallback -probe-all-ips -random-agent -o domain-httpx-out -oa
```
...
```bash
waymore -i lululemon.com -mode B -oU ./waymoreUrls.txt -oR ./waymoreResponses --notify-discord
```
